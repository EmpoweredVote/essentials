...
> ⚙️ **EXECUTION NOTES for plans 222-11 … 222-18 (rev 2, updated 2026-07-26 from waves 5–10).** These are environment facts, not preferences. **Rev 1 covered waves 2–4; items 5 and 6 in that revision are now WRONG and have been replaced.**
>
> **A. Dispatch and capability facts (unchanged, still true)**
...
> 2. **MCP tools are unbound in every subagent session.** All production SQL — live scope derivation *and* the migration apply — runs orchestrator-side. Never write a plan task that assumes the executing agent can query the DB.
> 3. **Re-derive scope live at execution.** 222-04's politician_ids were wrong for 4 of 15 people. Waves 5–10 re-derived every plan's scope orchestrator-side and it caught a real error: **222-08's plan prose named 8 mayors but the live scope was 5** (Murphy/Princeton/Melissa mayors already stanced, out of scope under D-07). Unchecked, that would have caused three duplicate passes and three candidate overwrites.
> 4. **Grep every subagent SUMMARY for verification claims before accepting it.** Agents in this phase have asserted checks that never ran. Verification is the one thing that must not be delegated — a false claim of a passed gate suppresses the real check permanently.
>
> **B. 🔴 MIGRATION NUMBERING — rev 1's rule was actively broken**
> 5. **NEVER derive a migration number from the local `C:/EV-Accounts` checkout.** Rev 1 said to run `ls backend/migrations | ... | tail -1`. That is **wrong in a multi-session repo**: on 2026-07-26 the local checkout reported max **1423** while `origin/master` was at **1467** — stale by 17 files. Three Claude sessions push to that repo and it moved 1464 → 1467 inside a single conversation. **Always fetch and read `origin/master`:**
>     ```bash
>     cd "C:/EV-Accounts" && git fetch origin master \
>       && git ls-tree --name-only origin/master backend/migrations/ \
>          | sed 's|.*/||' | grep -oE '^[0-9]+' | sort -n | tail -1
>     ```
>     ⚠️ Anchor on `^[0-9]+` of the **basename**. A bare `grep -oE '[0-9]{4}'` matches any 4-digit run in the path and will return a **year** from a filename like `1072_seed_2026_statewide_general_candidates.sql`.
> 6. **Operator ruling 2026-07-26, agreed with the EV-Accounts session: phase 222 takes the band 1468–1490.** The next plan to author a migration claims **1468**, then 1469, 1470… through the remaining plans and close-out. **Re-check the max immediately before claiming anyway** — if it returns anything above 1467, start one past whatever it shows.
> 7. **Cross-repo workflow: commit phase-222 migrations to the branch `data/phase-222-stances`, never to `master`.** That branch exists and is pushed (`origin/data/phase-222-stances`). `master` has `autoDeploy: yes` → pushing it deploys accounts-api to Render **and carries every commit already ahead**, including two other sessions' in-flight backend code. Commit with `git -C "C:/EV-Accounts" add/commit`; never `cd` in; never `push` `master`; never `stash`/`reset`/`rebase` in that repo. Migrations 1416–1423 are all already on `origin/master`.
> 8. **There is NO migration-tracking table in production** — `information_schema.tables` has nothing matching `%migration%` (verified by the EV-Accounts session 2026-07-26). So headers claiming "deliberately NOT registered in `schema_migrations`" describe a distinction that does not exist. **Write instead:** "no migration runner exists; this file records SQL applied by hand via `mcp__supabase-local__execute_sql`." "Applied" can only mean *its data effects are live*.
>
> **C. Source intelligence — what works and what is CLOSED**
> 9. **⚠️ BALLOTPEDIA WORKS, but only with a browser User-Agent and only via `curl`.** For seven waves this phase recorded Ballotpedia pages as "empty bodies" and treated that as absence of evidence. **It was a User-Agent block.** Control test: default agent → **HTTP 403, 919 bytes**; Chrome UA → **HTTP 200, 105,487 bytes**. **`WebFetch` cannot fetch ballotpedia.org at all — it always gets the 403.**
>     ```bash
>     UA="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36"
>     curl -s -A "$UA" "https://ballotpedia.org/Firstname_Lastname" -o out.html
>     ```
>     Strip tags before searching — text is entity-encoded (`&#39;`, `&#160;`) and split across tags, so a naive grep misses it. A real page is 90KB–250KB; **a ~51KB body with a `<title>` but no election tables is Ballotpedia's 404 stub**, meaning the person has no page — that is not evidence of anything. Survey content sits under a "Campaign themes" heading.
> 10. **⚠️ A Ballotpedia 200 IS NOT IDENTITY CONFIRMATION — this unlock introduced a new hazard.** Under the old 403 a wrong-person page was unreachable and harmless; now it returns confidently and in depth. Two real instances: **`ballotpedia.org/Jim_Atchison` is a Mississippi Republican** (2011 MS House D-116) and **`ballotpedia.org/Jessica_Walden` is a Georgia House District 144 Democrat** — both with populated campaign-themes sections **and party labels**, in a phase where party is never used and never displayed. **Confirm office, state and city on the page itself before using one word of it.** A disambiguation page (as `Kevin_Kelley` returns) is the benign case; the dangerous case is the one that looks certain.
> 11. **Ballotpedia's actual yield is LOW — check it, don't bank on it.** Across Plano's 4 newly-unlocked Candidate Connection surveys (25 pairs) and Anna/Murphy's 7 people, it produced **0 chairs**. The Plano surveys turned out to be biography, three-key-message lists, endorsements and one Isaac Asimov reference; Kehr's answers duplicated a Community Impact Q&A 222-04 had already read. **The seven-wave block cost the phase essentially nothing** — and most small-town officials here are below Ballotpedia's coverage scope entirely (genuine 404 stubs confirmed for Schulmeister, Dorian, Doug Charles, Shea Scott, Lee Pettle).
> 12. **CLOSED sources — do not spend time, do not carry as retries:**
>     - **VOTE411 / `onyourballot.vote411.org`** — escalated to a real headless browser with full JS; still 403 with page title `Voter Guide Toolkit: Forbidden Page`. That branded signature means **post-election guides are decommissioned, not gated**. Usable only for *upcoming* races, never for the past elections that seated these officials.
>     - **`citizenportal.ai`** — reachable but banner-labelled AI-generated. Not citable under D-05.
>     - **`gathergov.com`** — 403, and likely the same AI-synthesis class.
>     - **`lwvcollin.org`** — 403 throughout the phase.
> 13. **CivicClerk OData is the key to SPA-only minutes portals.** When a CivicClerk portal renders an empty SPA, its **unauthenticated OData API** still serves everything. This recovered Fairview's minutes (222-08) and Anna's entire corpus (222-10: 400 events back to 2024-03, 119 agendas, 9 packets). Pattern:
>     `https://<city>.api.civicclerk.com/v1/Events?$filter=…` — **key access `Events(id)` 404s; you must `$filter`** — each event carries an inline `publishedFiles[]`, then `GetMeetingFileStream(fileId=N,plainText=false)`. **Minutes often ride inside "Agenda Packet" PDFs as consent attachments.** Before concluding a city has no minutes, try AgendaCenter, Laserfiche WebLink, CivicClerk (+OData), Granicus, and a plain `/minutes`/`/agendas` path.
>
> **D. Attribution controls — every one of these caught a real false positive**
> 14. **⚠️ READ THE PRIMARY DOCUMENT; CONFIRM THE SPEAKER BY NAME.** Within-city misattribution — where the city is right and only the speaker is wrong — has hit this phase **five times**, and no homonym/city gate catches it. Real instances: a WebSearch summary gave **Doug Charles** a perfect quotable multifamily position that belonged to **Councilmember Marcus E. Ray** (the June 9 2026 minutes PDF disproved it); Anna growth quotes that belong to **former mayor Nate Pike (2021)**; Parker's entire Restore-the-Grasslands file, which every primary document assigns to **Pilgrim/Sharpe/Barron**, not the mayor; a Fairview vote story quoting Lessner/Custer/Doi while the dissenter's Nay had to be recovered from the minutes. **Mayors are structurally the most exposed** — they preside, so summaries credit the body's action to whoever chaired it. **A mayor calling a vote is not a mayor holding a position.** Always check the date falls inside that person's term.
> 15. **Extract `/Author` and `dc:creator` from unsigned municipal PDFs before attributing them.** A Lowry Crossing city newsletter arguing explicitly for standing up a police department — a clean chair 4 on the merits — was authored by **Janis Cable, the City Secretary**. A city's institutional voice is not an officeholder's position. Decisive twice.
> 16. **Read the staff report behind a motion before scoring the motion.** In 222-10, Kelley **moved** and Ison **seconded** "the addition of 1 FTE position within the Police Department" (7-0) — a named, affirmative, on-topic sponsorship for two people at once. The staff report shows a **backfill for an officer on 12-month military leave**, "no anticipated financial impact to the General Fund." Maintenance, not an increase. An agenda line states what was voted on, not what it means.
> 17. **A zero-hit grep on a PDF is a tooling result, not a factual one.** Municipal PDFs are frequently FlateDecode-compressed, so *no* body phrase is greppable — not even "Property Tax". Render or extract before concluding a passage is absent.
> 18. **`curl` bypasses WebFetch's ~15-minute per-URL cache**, making it the clean tool for a genuine re-verification read. A same-minute WebFetch re-fetch is not an independent read; say so if you rely on one.
>
> **E. Evidence-bar refinements established in waves 5–10**
> 19. **`taxes` — extended refusal list.** Beyond the standing ruling, none of these is taxes evidence either: **utility/water/wastewater/trash rate and fee decisions** (a rate is not a tax); **appraisal-district valuation complaints** ("the county raised your values" is not a position); **"more services at no additional cost"** promises (reach no chair in either direction); and **"expand the commercial tax base to relieve homeowners"** (refused for two Plano members — do not create a cross-city inconsistency).
> 20. **Shared vocabulary is not shared subject.** Fairview's mayor gave a real, dated, primary-sourced reason for a named Nay vote — *"It just doesn't fit into the character of the town of Fairview. It's just too big"* — and it is still a **blank** for `residential-zoning`, because the vote concerned a **non-residential religious building's steeple height** and every chair on that scale is a **housing-density** proposition. The word "character" appearing in both quote and topic question is lexical coincidence. Same logic refused a "keep pace while protecting the character of our community" line across three scales.
> 21. **Further blanks that look like chairs:** an **unexplained dissenting vote** cannot locate a chair (a No with no stated reason cannot distinguish the chairs it might imply — Shannon Moore, Ordinance 4495); **contradiction is a blank, not a tiebreak** (Doug Charles's platform said "NOT apartments" while he voted 6-0 for 86 townhomes by right); **maintenance framing** is not growth-pace management or a transportation mode tradeoff (built-out-city infrastructure, pothole repair, a bond bundling roads with drainage/water); **relaying constituent perception** is not the member's own position; **board/committee service, profession, career, tenure, academy participation, volunteering and forum attendance** are adjacency; and a **capital project** delivered during a tenure is not a funding-level position.
> 22. **A demotion is the resample contract working, not a failure.** Waves 6, 7, 9 and 10 each produced one. Report them explicitly — they are the most valuable output of a low-yield pass.
>
> **F. Scope facts for the remaining plans**
> 23. **Blank rates are high and correct — recalibrate expectations downward.** The phase stands at **16 chairs from ~460 attempted pairs**, and waves 8, 9, 10 plus the Plano gap-closure insert produced **0 chairs from 245 pairs**. `healthcare`, `local-immigration`, `civil-rights` and `growth-and-development` are near-total blanks for small-town officials; in Anna and Murphy, `homelessness` and `local-immigration` are **structurally** absent (zero such items exist anywhere in a 119-agenda corpus). **0–3 chairs from a 10-person plan is the expected, successful outcome.** Never lower the bar to avoid returning zeros.
> 24. **Classify every zero as `settled` or `access failure`** in the register. Settled = the complete relevant sources were read and there is genuinely no position, ideally with a structural reason (Anna's and Fairview's minutes are **action-only** — mover, seconder, tally, no reasoning — which is why those cities cannot yield positions from minutes). Access failure = name the exact URL, the failure mode, and the retry path. Waves 9 and 10 achieved all-settled zeros and that is what made all-blank plans genuinely useful rather than merely empty.
> 25. **An all-blank plan authors NO migration and claims NO number.** Write only register sections plus a close-out block stating the number stays unclaimed. Do not author an empty migration to have an artifact.
> 26. **People carrying prior 2026-05-12 found-nothing notes** (8 `inform.politician_context` rows each, no answer rows — deliberate honest-blank notes, not defects, and they do not trip the evidence-integrity gate): **Melissa 6 of 6** un-stanced (222-12), **Princeton 2 of 3** — Todd and Washington (222-11). Anna's were cleared in 222-10. A blank you reach independently **corroborates** the prior pass; say so. Replacing a found-nothing note with genuinely-sourced evidence is a legitimate **upgrade** (the Lavine precedent), but never lower the bar to overwrite a prior honest blank.
> 27. **222-11 is split into two dispatches** — Fairview's 6, then Princeton's 3 — after a single 9-person dispatch stalled mid-stream on an API error having committed nothing. **Commit per city, or more often.** Bound the blast radius; an agent can die at any moment.
> 28. **Register heading convention vs the plans' literal greps.** Sections are headed `## City of {Name} ({geo_id}) — 222-NN`, so the plans' `<automated>` checks that grep `^## {City}` do **not** match. The artifact contract is satisfied; the literal grep is wrong. **222-18 must reconcile on city name, not the `^## {City}` anchor.**
> 29. **`coverage.js` Pitfall 5 has NOT been triggered by any plan in this phase** — no government has moved 0 → ≥1 stance. Every city touched already carried `hasContext: true` (verified orchestrator-side for Allen, Richardson, Prosper, Celina, Longview). 222-18 should confirm rather than assume.
