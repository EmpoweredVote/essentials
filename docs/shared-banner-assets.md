...
### It exists now: `/banners.json`

**https://essentials.empowered.vote/banners.json** — generated from the registry, keyed by bucket
path, regenerated by `npm run banners:json` and guarded in CI by `npm run banners:check`, so it
cannot go stale without failing the build. **Stop parsing the comment block.**

```json
{
  "bucket_base": "https://.../politician_photos/",
  "counts": { "assets": 236, "credited": 233, "uncredited": 3, "unmatched_credits": 0 },
  "assets": [
    { "path": "cities/alexandria.jpg", "kind": "city", "state": "VA",
      "title": "Old Town King St historic storefronts", "author": "DiscoA340",
      "license": "CC BY-SA 4.0" }
  ],
  "uncredited": [ { "path": "cities/portland-me.jpg", "kind": "city", "state": "ME" } ],
  "unmatched_credits": []
}
```
...
- **`path` is the key, not the place name.** Portland OR and Portland ME are both keyed `portland`
  in our code; only `cities/portland.jpg` vs `cities/portland-me.jpg` separates them. Join on
  `path`. Never on a name.
- **`license` is a closed vocabulary** (`CC BY-SA 4.0`, `CC0 / Public Domain`, ...). Production
  notes that used to sit in that field — `(anchor .85)`, `[brightened]`, crop prose — are split out
  into optional `license_note`. Display `author` and `license` verbatim; `license_note` is ours, not
  the licensor's.
- **`uncredited` is a promise, not an oversight.** Those assets ship with no credit line in the
  registry, so we publish no author for them rather than a plausible one. If you cannot display an
  asset without attribution, skip exactly this list. It is currently three: `portland-me`,
  `springfield-mo`, `west-valley-city`.
- **`unmatched_credits` should always be empty.** A non-empty array means a credit exists that we
  could not attach to a shipping asset — our bug, and worth telling us about.
