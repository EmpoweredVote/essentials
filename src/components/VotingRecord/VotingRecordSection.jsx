    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden">
      ...
      {!loading && total > 0 && (
        <div className="px-5 pt-3 flex gap-2 flex-wrap">
          {['ALL', 'YES', 'NO', 'ABSENT', 'ABSTAIN'].map(f => (
            <button
              key={f}
              onClick={() => { track('essentials_voting_record_filter_changed', { filter: f }); setFilter(f); setOffset(0); }}
          ...
          <ul className="divide-y divide-gray-100 dark:divide-gray-800">
            {votes.map((v, i) => (
              <li
                ...
                onClick={() => { if (v.council_file_number) { track('essentials_voting_record_bill_opened'); setSelectedFile(v.council_file_number); } }}
                ...
                onKeyDown={(e) => {
                  if (v.council_file_number && (e.key === 'Enter' || e.key === ' ')) {
                    e.preventDefault();
                    track('essentials_voting_record_bill_opened');
      ...
      {!loading && filteredTotal > LIMIT && (
        <div className="px-5 pb-4 flex items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-3">
          <button
            onClick={() => { track('essentials_voting_record_page_changed', { direction: 'prev' }); setOffset(Math.max(0, offset - LIMIT)); }}
            disabled={offset === 0}
          ...
          <button
            onClick={() => { track('essentials_voting_record_page_changed', { direction: 'next' }); setOffset(offset + LIMIT); }}
