...
- **After every plan wave:** Run the cross-wave integrity SELECT (bidirectional office↔politician links, ONE chamber, official_count=6 — 6 council district seats; the directly-elected LOCAL_EXEC Mayor is a separate exec office and is NOT counted in the council chamber's official_count)
- **Before `/gsd:verify-work`:** Wave-4 final reconciliation block must return all-green (Mayor + 6 districted council seats D1–D6 = 7 offices total, council chamber official_count=6, 0 duplicate/stale office rows, 0 uncited/0 unpaired stances, split-section check = 0 rows)
